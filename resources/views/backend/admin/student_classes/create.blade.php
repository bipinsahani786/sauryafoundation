<x-dashboard.layout>
    <x-slot name="title">Create Academic Class</x-slot>

    <div class="mb-8 flex justify-between items-center">
        <div>
            <h2 class="text-xl font-black text-slate-900 tracking-tight">Register New Class</h2>
            <p class="text-xs text-slate-400 font-bold italic">Define a global student category.</p>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden p-10 max-w-2xl">
            <div class="space-y-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" placeholder="e.g. Class 10, SSC, UPSC Level 1" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-xs font-bold text-slate-900 focus:border-indigo-600 outline-none placeholder:text-slate-300" required>
                    @error('name')
                ...
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex items-center gap-3 p-4 bg-slate-50 border border-slate-200 rounded-2xl cursor-pointer hover:border-indigo-600 transition-all group">
                            <input type="radio" name="status" value="active" checked class="w-4 h-4 rounded-full border-slate-300 text-indigo-600 focus:ring-indigo-600">
                            <div>
                        ...
                        <label class="relative flex items-center gap-3 p-4 bg-slate-50 border border-slate-200 rounded-2xl cursor-pointer hover:border-rose-600 transition-all group">
                            <input type="radio" name="status" value="inactive" class="w-4 h-4 rounded-full border-slate-300 text-rose-600 focus:ring-rose-600">
                            <div>
                ...
                <div class="pt-8 border-t border-slate-100 flex justify-end">
                    <button type="submit" class="bg-slate-900 text-white px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl shadow-slate-100">
                        Confirm & Save Class <i class="fas fa-check ml-2"></i>
                    </button>
                </div>
            </div>
        </div>
