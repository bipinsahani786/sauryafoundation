<x-dashboard.layout>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-3xl">
        <form action="{{ route('admin.admit-cards.bulk-store') }}" method="POST" class="p-6">
            @csrf
            ...
            new TomSelect("#quiz_select", {
                ...
                render: {
                    ...
                    option: function(data, escape) {
                        if (data.title) {
                            return '<div><span class="font-bold">' + escape(data.title) + '</span> <span class="text-slate-400 text-xs ml-1">(' + escape(data.enrolled) + ' enrolled)</span></div>';
                        }
                        return '<div>' + escape(data.text) + '</div>';
                    }
                }
