<x-dashboard.layout>
    ...
    <div class="mb-6 flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
            ...
            <p class="text-xs text-slate-400 font-bold mt-1">Verify and authorize monetized assessments from teachers.</p>
        </div>
    </div>
                ...
                @forelse($quizzes as $quiz)
                    <tr class="hover:bg-slate-50">
                        <td class="px-6 py-4">
                            ...
                            <div class="text-[9px] text-indigo-600 font-black uppercase tracking-widest">By Profile: {{ $quiz->teacher->name ?? 'Admin' }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            @if($quiz->status == 'pending')
                                <div class="flex justify-end gap-2">
                                    <form action="{{ route('admin.quiz-approvals.approve', $quiz->id) }}" method="POST">
                                        @csrf
                                    ...
                                    <form action="{{ route('admin.quiz-approvals.reject', $quiz->id) }}" method="POST">
                                        @csrf
