<x-dashboard.layout>
    <x-slot name="title">Update Global Resource | Shaurya Narayan Foundation</x-slot>
    ...
    <div class="mb-10 flex items-center gap-6 italic">
        ...
        <div>
            <h2 class="text-3xl font-black text-slate-900 tracking-tight uppercase">Edit Resource Node</h2>
            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em] leading-loose">Modify the parameters of an existing global academic resource.</p>
        </div>
    </div>

    <div class="grid lg:grid-cols-3 gap-10 italic">
        <div class="lg:col-span-2">
            <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden p-12">
                <form action="{{ route('admin.study-materials.update', $studyMaterial) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                    
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Resource Title</label>
                        <input type="text" name="title" value="{{ old('title', $studyMaterial->title) }}" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold focus:border-indigo-600 focus:bg-white outline-none transition-all placeholder:text-slate-300 uppercase" required>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" rows="4" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold focus:border-indigo-600 focus:bg-white outline-none transition-all placeholder:text-slate-300">{{ old('description', $studyMaterial->description) }}</textarea>
                    </div>
                    ...
                    <div class="p-10 border-2 border-dashed border-slate-200 rounded-[2.5rem] bg-slate-50 text-center hover:border-indigo-300 hover:bg-indigo-50/50 transition-all group overflow-hidden relative">
                         <label class="cursor-pointer">
                            ...
                            <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-indigo-600 transition-colors">Replace Secure Node</span>
                            <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-2 block italic opacity-50 uppercase">Leave empty to keep current file: {{ basename($studyMaterial->file_path) }}</span>
                    ...
                    <div class="pt-6 flex justify-end">
                        <button type="submit" class="bg-indigo-600 text-white font-black py-4 px-12 rounded-2xl text-[10px] uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl shadow-indigo-100 italic group active:scale-95">
                            Update Resource Hub <i class="fas fa-save ml-2 text-[8px] group-hover:scale-125 transition-transform"></i>
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden h-fit italic">
                ...
                <h4 class="text-xl font-black italic tracking-tighter uppercase mb-6 relative z-10">Update Protocol</h4>
                <div class="space-y-6 relative z-10 font-bold text-sm leading-relaxed text-slate-400">
                    <p class="text-indigo-400">1. Replacing the file will permanently delete the previous version from the secure vault.</p>
                    <p>2. Modifications take effect instantly across all student terminals.</p>
                </div>
            </div>
        </div>
    </div>
